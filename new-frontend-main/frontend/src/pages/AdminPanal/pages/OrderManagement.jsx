...
export default function OrdersManagement() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      // Flatten all user orders into a single list with user info
      let allOrders = [];
      users.forEach((u) => {
  ...
  const updateOrderStatus = async (orderId, customerEmail, newStatus) => {
    try {
      // Fetch the customer from backend
  ...
  if (!isAdmin) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#FAF9F6]">
        <h2 className="text-lg text-red-500">Access Denied</h2>
      </div>
    );
  }
      ...
      <div className="flex-1 p-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-[#E5D9C5] p-6">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-[#5A5A5A]">No orders found</div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-[#E5D9C5]">
                <thead className="bg-[#F8F5F0]">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-[#5A5A5A]">
                      Customer
                    </th>
                    <th
